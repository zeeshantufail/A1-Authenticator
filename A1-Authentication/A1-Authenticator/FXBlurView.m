...
- (void)setUp
{
    ...
    if (!_blurRadiusSet) [self blurLayer].blurRadius = 0;
...
{
    if([snapshot respondsToSelector:@selector(blurredImageWithRadius:iterations:tintColor:)])
    {
...
- (void)updateAsynchronously:(BOOL)async completion:(void (^)())completion
{
    ...
    {
        UIImage *snapshot = self.videoFrameImage;  //[self snapshotOfUnderlyingView];
        if(!self.videoFrameImage)
        {
            return;
        }
        //self.videoFrameImage = nil;
        if (async)
        {
